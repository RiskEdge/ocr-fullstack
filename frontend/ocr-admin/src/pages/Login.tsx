...
export default function Login() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const companyName = mode === 'company'
  ...
  const fieldCls = 'w-full px-4 py-2.5 rounded-md border border-gray-200 text-sm text-gray-900 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent focus:bg-white transition-colors'
      ...
      <div className="w-full max-w-md">
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="bg-white/10 backdrop-blur-sm rounded-lg px-6 py-4 mb-5 flex flex-col items-center gap-2">
            <img src="/riskedge.png" alt="Risk Edge" className="h-9 w-auto brightness-0 invert" />
          </div>
          ...
          <div className="flex rounded-md border border-gray-200 overflow-hidden mb-6">
            {(['global', 'company'] as const).map(m => (
              <button
                key={m}
                type="button"
                onClick={() => { setMode(m); setError(''); setCompanyId('') }}
              ...
              >
                {m === 'global' ? 'Partner / Risk Edge' : 'Company Admin'}
              </button>
            ))}
          </div>
