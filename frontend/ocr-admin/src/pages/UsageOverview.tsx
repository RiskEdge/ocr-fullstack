...
export default function UsageOverview() {
  ...
  const partnerOptions = useMemo(
    () => isSuperAdmin
      ? [...new Set((data?.by_company ?? []).map(c => c.partner_name).filter(Boolean))].sort()
      : [],
    [data, isSuperAdmin],
  )
  ...
  const companyOptions = useMemo(() => {
    ...
    const filtered = partnerFilter ? source.filter(c => c.partner_name === partnerFilter) : source
  ...
  const filteredCompanies = useMemo(
    () => (data?.by_company ?? []).filter(c =>
      !partnerFilter || c.partner_name === partnerFilter
    ),
    [data, partnerFilter],
  )
  ...
  const companyColumns: Column<CompanyUsage>[] = [
    ...
    ...(isSuperAdmin ? [{ key: 'partner_name' as const, header: 'Partner', sortable: true }] : []),
    { key: 'ocr_runs',      header: 'OCR Runs',      sortable: true },
    { key: 'ocr_pages',     header: 'OCR Pages',     sortable: true },
    { key: 'ocr_credits',   header: 'OCR Credits',   sortable: true },
    { key: 'val_runs',      header: 'Val Runs',      sortable: true },
  ...
  return (
    <div className="p-8 space-y-6 max-w-full">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Aggregated activity by partner, company and user</p>
      </div>
      ...
      <div className="flex gap-1 bg-gray-100 rounded-lg p-1 w-fit">
        {isSuperAdmin && (
          <button
            ...
            className={cn(
              ...
              tab === 'partner' ? 'bg-white text-indigo-700 shadow-sm' : 'text-gray-500 hover:text-gray-700',
            )}
          >
        ...
        <button
          ...
          className={cn(
            'flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all',
            tab === 'company' ? 'bg-white text-indigo-700 shadow-sm' : 'text-gray-500 hover:text-gray-700',
          )}
        >
        ...
        <button
          ...
          className={cn(
            'flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all',
            tab === 'user' ? 'bg-white text-indigo-700 shadow-sm' : 'text-gray-500 hover:text-gray-700',
          )}
        >
