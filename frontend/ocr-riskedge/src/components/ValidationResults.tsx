...
import type { ValidatedItem, Discrepancy, ValidationResult, PluOption } from '@/lib/validateApi';
...
function computeLocalValidation(
	item: ValidatedItem,
	master: PluOption,
): { discrepancies: Discrepancy[]; corrections: Record<string, number | string> } {
...
const ValidationResults = ({ items, documentScalars }: Props) => {
	...
	const stats = useMemo(() => {
		...
		for (let idx = 0; idx < items.length; idx++) {
			...
			const effectiveDiscrepanciesRaw =
				v.match_type === 'multi_plu' && pluSel ? pluSel.discrepancies : v.discrepancies;
			...
			const effectivelyValid =
				(v.match_type === 'multi_plu' ? pluSel!.discrepancies.length === 0 : v.is_valid) ||
				remaining === 0;
	...
	const calcResults = useMemo((): CalcValidationResult | null => {
		...
		for (let idx = 0; idx < items.length; idx++) {
			...
			const effectiveCostPrice = parseFloat(String(effectiveItem['cost_price'] ?? ''));
			const effectiveTaxPct = parseFloat(String(effectiveItem['tax_pct'] ?? ''));
			...
			if (!isNaN(effectiveCostPrice) && !isNaN(quantity) && lineAmtField) {
				const calculated = parseFloat((effectiveCostPrice * quantity).toFixed(2));
				checks.push({
					label: 'Line Amount',
	...
	function selectPlu(itemIdx: number, opt: PluOption) {
		const { discrepancies, corrections } = computeLocalValidation(items[itemIdx], opt);
	...
	function downloadValidationCsv() {
		...
		const rows = items.map((item, idx) => {
			...
			// Matched PLU
			const matchedPlu =
				v.match_type === 'multi_plu' ? (pluSel?.plu_code ?? '') : (v.matched_plu ?? '');
												...
												<tr>
													<td colSpan={totalCols} className='p-0'>
														<div className='bg-muted/10 border-t border-border px-4 py-3 space-y-3'>
															...
															{(() => {
																const lr =
																	calcResults?.lineResults.find(
																		(r) => r.idx === idx,
																	);
																if (!lr || lr.checks.length === 0)
																	return null;

																const failingChecks = lr.checks.filter((c) => !c.ok);
																const allOk =
